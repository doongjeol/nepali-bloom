...
function buildExercise(card: GrammarPracticeCard): Exercise {
  ...
  if (kind === "ergative-le") {
    ...
    return {
      ...
      prompt: "과거 타동사 문장에서 주어에 붙는 마커를 고르세요:\nma____ kitaab kholeko.",
      choices,
      answer: correct,
...
export function GrammarExercise({ card }: { card: GrammarPracticeCard }) {
  const [exercise, setExercise] = useState<Exercise>(() => buildExercise(card));
  ...
  const hasNext = useMemo(() => {
    ...
    return false;
  }, [card, exercise.prompt, exercise.answer]);
