...
export function RangeSelector({ min, max, initialStart, initialEnd, onSubmit, className }: RangeSelectorProps) {
  ...
  return (
    <div className={cn("w-full max-w-full rounded-2xl border bg-card p-4 shadow-sm", className)}>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <label className="space-y-1">
          ...
          <input
            ...
            step={1}
            inputMode="numeric"
            className="w-full rounded-xl border bg-background px-3 py-2.5 text-base outline-none focus:ring-2 focus:ring-ring/40"
            placeholder={`${min}`}
          />
        </label>
        <label className="space-y-1">
      ...
      <button
        type="button"
        className="mt-4 w-full rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground hover:bg-primary/90 active:scale-[0.99] transition-all disabled:opacity-60"
